...
class Storage:
    ...
    def detailed_stats(self, inactivity_seconds:int=1800) -> dict[str,Any]:
        ...
        base['unfinished_leads']=self._one("SELECT count(*) FROM attempts WHERE status<>'completed' AND amo_lead_id IS NOT NULL")[0]
        return base
    ...
    def unfinished_leads(self, limit:int=10, offset:int=0)->list[sqlite3.Row]:return self.db.execute("SELECT id,full_name,phone,amo_lead_id,started_at FROM attempts WHERE status<>'completed' AND amo_lead_id IS NOT NULL ORDER BY id DESC LIMIT ? OFFSET ?",(limit,offset)).fetchall()
